...
import EsewaPayment from "../components/Esewa/EsewaPayment";
  ...
  const placeOrder = async () => {
    try {
      ...
      if (paymentOption === "cod") {
        const { data } = await api.post("/order/cod", orderPayload);
        if (data.success) {
    ...
    <div className="flex flex-col md:flex-row mt-30">
      <div className="flex-1 max-w-4xl">
        ...
        {cartArray.map((product, index) => (
          <div
            key={index}
            className="grid grid-cols-[2fr_1fr_1fr] items-center pt-3 text-sm md:text-base font-medium text-gray-500"
          >
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
        ...
        <div className="mb-6">
          ...
          >
            ...
            <option value="esewa">Online Payment</option>
          </select>
          {paymentOption === "esewa" && (
  <EsewaPayment
  amount={getCartAmount() + getCartAmount() * 0.02}
  productId={cartArray.length > 0 ? cartArray[0]._id : "default-product-id"}
/>
)}
        </div>
